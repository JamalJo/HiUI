...
public class RefreshableView extends RelativeLayout {
     ...
     * @param id       为了防止不同界面的左拉加载更多在上次更新时间上互相有冲突， 请不同界面在注册左拉加载更多监听器时一定要传入不同的id。
     */
    ...
    private void setIsAbleToPull(MotionEvent event) {
        LinearLayoutManager manager = (LinearLayoutManager) refreshView.getLayoutManager();
        if (manager == null) {
            return;
        }
        ...
        Log.d(TAG, "findLastCompletelyVisibleItemPosition: "
                + manager.findLastCompletelyVisibleItemPosition() + " getItemCount: "
